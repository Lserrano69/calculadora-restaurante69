...
const OrderPanel: React.FC<OrderPanelProps> = ({
  ...
  const orderTotal = useMemo(() => {
    return Object.values(order).reduce((total: number, { item, quantity }) => total + item.price * quantity, 0);
  }, [order]);
  ...
  return (
    <>
      ...
      />
      <div className="lg:w-1/3 bg-gray-900 p-5 rounded-lg shadow-inner border border-gray-700 flex flex-col max-h-[75vh]">
        <h2 className="text-2xl font-bold text-green-400 mb-5 text-center">Pedido Actual</h2>
        <div id="current-order-container" className="flex-1 overflow-y-auto pr-2 mb-4">
          {orderIsEmpty ? (
        ...
        <div className="mt-auto border-t border-gray-700 pt-4">
           ...
           <div className="mb-6 pt-4 border-t border-gray-700">
                <h3 className="text-xl font-semibold text-indigo-400 mb-3 text-center">Gestión Rápida del Menú</h3>
                <form onSubmit={handleAddMenuItem} className="flex flex-col sm:flex-row gap-3">
                    <input type="text" value={newItemName} onChange={e => setNewItemName(e.target.value)} placeholder="Nombre del Producto" className="flex-1 p-2 rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:ring-indigo-500 focus:border-indigo-500" required />
                    ...
                    <button type="submit" disabled={isAdding} className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition duration-200 ease-in-out transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <span id="total-amount" className="text-3xl font-bold text-green-400">${orderTotal.toFixed(2)}</span>
            </div>
            <div className="mb-4">
              <label htmlFor="received-amount" className="block text-gray-300 text-lg font-semibold mb-2">Monto Recibido:</label>
              <input type="number" id="received-amount" placeholder="0.00" step="0.01" min="0" value={receivedAmount} onChange={e => setReceivedAmount(e.target.value)} className="w-full p-3 rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 text-2xl focus:ring-indigo-500 focus:border-indigo-500" />
            </div>
            <div className="flex justify-between items-center mb-6">
              <span className="text-xl font-semibold text-gray-300">CAMBIO:</span>
              <span id="change-amount" className={`text-3xl font-bold ${changeAmount < 0 ? 'text-red-400' : 'text-yellow-400'}`}>
                ${(isNaN(changeAmount) ? 0 : changeAmount).toFixed(2)}
              </span>
            </div>
            <button onClick={() => setConfirmClear(true)} disabled={orderIsEmpty} className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 rounded-lg shadow-md transition duration-200 ease-in-out transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
              Limpiar Pedido
            </button>
        </div>
      </div>
