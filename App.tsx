...
function App() {
  ...
  const addMenuItem = async (name: string, price: number) => {
    if (!userId) {
      showMessage('Debes estar conectado para añadir productos.', 'error');
      return;
    }
    try {
      await addMenuItemToDB(userId, name, price);
  ...
  const deleteMenuItem = async (itemId: string) => {
    if (!userId) {
      showMessage('Debes estar conectado para eliminar productos.', 'error');
      return;
    }
    try {
      await deleteMenuItemFromDB(userId, itemId);
       // Remove from current order if it exists
      setCurrentOrder(prevOrder => {
        const newOrder = { ...prevOrder };
        if (newOrder[itemId]) {
          delete newOrder[itemId];
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-4 font-sans">
        <div className="bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-6xl flex flex-col lg:flex-row gap-6 border border-gray-700">
          <div className="lg:w-2/3 bg-gray-900 p-5 rounded-lg shadow-inner border border-gray-700">
            ...
            {isLoading ? (
              ...
              <div id="menu-items-container" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 max-h-[60vh] overflow-y-auto pr-2">
                {menuItems.length > 0 ? (
                  menuItems.map((item) => (
                    <MenuItemCard
                      key={item.id}
                   ...
                   <p className="text-gray-400 text-center col-span-full py-4">No hay productos en el menú. Añade algunos para empezar.</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="mt-6 text-center text-gray-400 text-sm">
          <p>ID de Usuario: <span className="font-mono text-gray-300">{userId || 'Cargando...'}</span></p>
